...
def peak_posterior(emission_probabilities, environment_size, segments, spikemats, *,
    ...
    """Per-position peak-phase histogram (Wang et al. 2020, Fig. 1B style).
    ...
    params = dict(bin_size_cm=bin_size_cm, phase_bin=phase_bin, minimum_max=minimum_max,
                half_width=half_width, velocity_cutoff=velocity_cutoff,
                spike_count_percentile=spike_count_percentile, count_window=count_window,
                theta_start=theta_start, theta_length_s=theta_length_s, direction=direction)
    return {"dist_of_peak_post": dist_of_peak,
            "norm_dist": dist_of_peak / col,
            "n_oscillations_used": len(selected),
            "spike_count_threshold": thr,
            "params": params}
...
def _two_troughs(trace, phase_bin, smooth_sigma=1.0):
    ...
    if len(idx) == 0:
        raise ValueError("no troughs found")
